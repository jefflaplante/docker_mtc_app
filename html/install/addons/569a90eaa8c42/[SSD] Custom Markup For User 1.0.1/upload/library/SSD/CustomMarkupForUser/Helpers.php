...
class SSD_CustomMarkupForUser_Helpers {
    ...
    public static function assembleCustomMarkup($options, $category) {
        ...
        foreach ($options[$category] as $optionName => $optionValue) {
            foreach (SSD_CustomMarkupForUser_Constants::$availableMarkups[$optionName]["format"] as $tag) {
    ...
    public static function assembleCustomMarkupPermissionForUser($group) {
        ...
        foreach (SSD_CustomMarkupForUser_Constants::$availableMarkups as $markupName => $markupArray) {
            $finalPermissions[$markupName] = $visitor->hasPermission("SSD_cmfu", sprintf($markupArray["permission"], $titleCode));
    ...
    protected static function replacePlaceholders($type, $str, $value) {
        switch ($type) {
            case "_value":
                ...
                break;
            case "fontFamily":
                $str = self::strReplaceRecursive("{fontFamily}", SSD_CustomMarkupForUser_Constants::$fontList[$value]["fullname"], $str);
                break;
            case "borderStyle":
                $str = self::strReplaceRecursive("{borderStyle}", SSD_CustomMarkupForUser_Constants::$borderList[$value], $str);
                break;
        }
        return $str;
    }
    ...
    public static function prepareSerializedOptionsForView($options) {
        ...
        if (!$fullUserOptions) {
            $fullUserOptions = SSD_CustomMarkupForUser_Constants::$defaultOptionsArray;
        }
        foreach ($fullUserOptions as $category => $catArray) {
            foreach ($catArray as $itemName => $itemValue) {
                if (isset(SSD_CustomMarkupForUser_Constants::$availableMarkups[$itemName]["enable_prefix"])) { // This item has an enable_ marker, tick it as well
    ...
    public static function verifyBorderList($itemValue) {
        return in_array($itemValue, array_keys(SSD_CustomMarkupForUser_Constants::$borderList));
    }

    public static function verifyFontList($itemValue) {
        return in_array($itemValue, array_keys(SSD_CustomMarkupForUser_Constants::$fontList));
    }

...
{
$__compilerVar2 = '';
$__compilerVar2 .= '<li class="Alert listItem' . (($alert['new']) ? (' new') : ('')) . (($alert['unviewed']) ? (' unviewed') : ('')) . ' PopupItemLink' . (($alert['bookmark'] AND ($alert['action'] == ('edit') OR $alert['action'] == ('delete') OR $alert['action'] == ('merge'))) ? (' PopupItemLinkActive') : ('')) . '" id="alert' . htmlspecialchars($alert['alert_id'], ENT_QUOTES, 'UTF-8') . '" data-author="' . htmlspecialchars($alert['user']['username'], ENT_QUOTES, 'UTF-8') . '">

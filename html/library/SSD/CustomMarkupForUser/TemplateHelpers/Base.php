...
class SSD_CustomMarkupForUser_TemplateHelpers_Base extends XenForo_Template_Helper_Core {
    public static function helperUserName(array $user, $class = '', $rich = false) {
        if (!$rich and XenForo_Application::getOptions()->get("SSD_cmfu_overridePlainUsernameHelper")) {
            $rich = true;
        }
    ...
    public static function helperUserNameHtml(array $user, $username = '', $rich = false, array $attributes = array()) {
        ...
        if (!$rich and XenForo_Application::getOptions()->get("SSD_cmfu_overridePlainUsernameHelper") == 1) {
            $rich = true;
        }
        if (!isset($user["SSD_cmfu_options"])) {
            $user["SSD_cmfu_options"] = XenForo_Application::getDb()->fetchOne("SELECT `SSD_cmfu_options` FROM `xf_user` WHERE `user_id`=?", $user["user_id"]);
        }
        if (XenForo_Application::getOptions()->get("SSD_cmfu_useCache") && !isset($user["SSD_cmfu_render_cache"])) {
            $user["SSD_cmfu_render_cache"] = XenForo_Application::getDb()->fetchOne("SELECT `SSD_cmfu_render_cache` FROM `xf_user` WHERE `user_id`=?", $user["user_id"]);
        }
        if ($rich) {
    ...
    public static function helperRichUserName(array $user, $usernameHtml = '') {
        $stylingPref = intval(XenForo_Application::getOptions()->get("SSD_cmfu_usernameStylingPreference"));
        ...
        if (empty($user["user_id"])) {
            $html = "{inner}";
        } elseif (XenForo_Application::getOptions()->get("SSD_cmfu_useCache")) {
            ...
            if (!empty($renderCache) && !empty($renderCache["username"])) {
                ...
                $html = SSD_CustomMarkupForUser_Helpers::assembleCustomMarkup($options, "username");
    ...
    public static function helperUserTitle($user, $allowCustomTitle = true, $withBanner = false) {
        ...
        if (XenForo_Application::getOptions()->get("SSD_cmfu_useCache")) {
            ...
            if (!empty($renderCache) && !empty($renderCache["usertitle"])) {
                ...
                $html = SSD_CustomMarkupForUser_Helpers::assembleCustomMarkup($options, "usertitle");
                XenForo_Model::create("XenForo_Model_User")->rebuildCustomMarkupCache($user["user_id"]);
            }
        } else {
            $html = SSD_CustomMarkupForUser_Helpers::assembleCustomMarkup($options, "usertitle");

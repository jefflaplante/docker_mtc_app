...
abstract class XenForo_ViewPublic_Helper_Editor
{
	...
	public static function supportsRichTextEditor()
	{
		...
		if (self::$_editorIds && preg_match('#android (\d+)\.(\d+)#i', $_SERVER['HTTP_USER_AGENT'], $match))
		{
			$showWysiwyg = false; // there's a problem with multiple editors on the same page in android
		}

		...
		if (uix.collapsibleSticky) {
			...
			if (stickyThreads.length) {
				var nextNum = uix.collapsible.nextItemNum(),
					ele = stickyThreads[0],
					trigger = stickyThreads[0].getElementsByClassName('uix_collapseNodes'),
					defaultVal = true,
...
uix.debug = function(skipMarks) {
	...
	result += uix.logVar(uix.offCanvasTriggerWidth, 'uix.offCanvasTriggerWidth');
...
uix.messageInfo = {
	...
	init: function() {
		...
		for (var i = 0, len = messages.length; i < len; i++) {
			...
			if (uix.threadSlidingExtra) {
				if ($('.threadSlidingAvatar', messages[i]).length === 0) {
					avatarHolder = true;
				}
			}
			...
			if (extraInfo || avatar) {
				$(messages[i]).addClass('uix_noCollapseMessage');
			}
		}

		if (uix.threadSlidingHover && !XenForo.isTouchBrowser()) {
			$('.uix_threadSlideToggle').mousemove(function(e) {
			...
			$('.uix_threadSlideToggle').mouseenter(function(e) {
				window.clearTimeout(um.hoverTimer);
				uix.messageInfo.hoverTimer = window.setTimeout(function() {
			...
			$('.uix_threadSlideToggle').mouseleave(function(e) {
				window.clearTimeout(um.hoverTimer);
			});
		...
		$('.uix_threadSlideToggle').click(function(e) {
	...
	collapse: function(ele, clicked) {
		var parent = $(ele).closest('.messageUserBlock'),
			collapseBlocked = false;

		if ($(parent).closest('.message').hasClass('uix_noCollapseMessage') || $(parent).closest('.message').hasClass('quickReply')) {
			collapseBlocked = true;
		}

		if (uix.isSet(parent) && uix.responsiveMessageBreakpoint < uix.windowWidth && parent.hasClass('expanded') && !collapseBlocked && (!parent.hasClass('recentChange') || clicked === true)) {
	...
	expand: function(ele, clicked) {
		if (uix.isSet(ele)) {
			...
			if (uix.isSet(parent) && uix.responsiveMessageBreakpoint < uix.windowWidth && !parent.hasClass('expanded') && !collapseBlocked && (!parent.hasClass('recentChange') || clicked === true)) {
				if (uix.threadSlidingAvatar) {
					$('.avatarHolder', parent).slideDown();
				}
				if (uix.threadSlidingExtra) {
					$('.extraUserInfo', parent).slideDown();
				}
	...
	initGet: function() {
		...
		if (um.eles.length) {
			if (uix.signatureHoverEnabled && !XenForo.isTouchBrowser()) {
				$('.uix_signatureToggleHover').mousemove(function(e) {
					window.clearTimeout(um.hoverTimer);
				...
				$('.uix_signatureToggleHover').mouseenter(function(e) {
					window.clearTimeout(um.hoverTimer);
					uix.messageSignature.hoverTimer = window.setTimeout(function() {
						var parent = um.getParent(e.target);
				...
				$('.uix_signatureToggleHover').mouseleave(function(e) {
					window.clearTimeout(um.hoverTimer);
				});

				$('li.message').mouseleave(function(e) {
					var parent = um.getParent(e.target);
					if (uix.isSet(parent)) {
			...
			$('.uix_signatureToggle').click(function(e) {
				var parent = um.getParent(e.target);
...
uix.offcanvas = {
	...
	initGet: function() {
		...
		uix.offcanvas.liHeight = $('.uix_sidePane .navTab .blockLinksList li').height();
		...
		$('.uix_sidePane .SplitCtrl').on('click', function(e) {
			if ($(e.target).closest('.navTab').hasClass('active')) {
				...
				$('.subMenu', $(e.target).closest('.navTab')).css('maxHeight', ($('.blockLinksList li', $(e.target).closest('.navTab')).length * uix.offcanvas.liHeight) + 'px');
			}
			return false;
		});
		...
		uix.offcanvas.sidePanes = uix.offcanvas.wrapper.find('.uix_sidePane');
	...
	initSet: function() {
		...
		$('.uix_sidePane .navTab.selected .subMenu').css('maxHeight', ($('.blockLinksList li', $('.uix_sidePane .navTab.selected')).length * uix.offcanvas.liHeight) + 'px');
...
uix.sidebar = {
	...
	initGeneral: function() {
		uix.sidebar.collapseEle = $('.uix_sidebar_collapse');
	...
	update: function() {
		if (uix.windowWidth > uix.sidebarMaxResponsiveWidth) {
			...
			if (uix.sidebarSticky.length && uix.elm.sidebarInnerWrapper.length && uix.sidebarSticky.running) {
	...
	*/
	...
	function injectElementWithStyles(rule, callback, nodes, testnames) {
		...
		if (body.fake) {
			...
			var test = docElement.offsetHeight;
		} else {
...
uix.userBar = {
	...
	init: function() {
		...
		if (uix.userBar.ele.length && uix.offCanvasSidebarVisitorTabs) {
			if ($('.moderatorTabs', uix.userBar.ele).length) {
				...
				for (var i = 0, len = visitorTabs.length; i < len; i++) {
					var className = visitorTabs[i].className;
					if ((className.indexOf('account') == -1 && className.indexOf('inbox') == -1 && className.indexOf('alert') == -1) || className.indexOf('uix_offCanvas_trigger') > -1) {
						uix.userBar.enableHiding = false;
					}
				}
			}
		...
		if (uix.userBar.enableHiding && !uix.userBar.userbarSet) {
			for (var i = 0, len = uix.sticky.items.length; i < len; i++) {
				if (uix.sticky.items[i].name === '#userBar') {
					var changeMade = false;
					if (uix.sticky.items[i].minWidth < uix.offCanvasTriggerWidth) {
						uix.sticky.items[i].minWidth = uix.offCanvasTriggerWidth;
						changeMade = true;
					}
					if (uix.sticky.items[i].minWidthPortrait < uix.offCanvasTriggerWidth) {
						uix.sticky.items[i].minWidthPortrait = uix.offCanvasTriggerWidth;
						changeMade = true;
					}
...
uix.toggleWidth = {
	...
	init: function() {
		if (uix.pageStyle == 2) {
			uix.toggleWidth.eles = $('#uix_wrapper, .pageWidth');
		} else {
...
uix.init.offsetAnchors = function() {
	...
	for (var i = 0, len = anchors.length; i < len; i++) {
		...
		if (href.indexOf('#') !== -1) {
			...
			if (split.length == 2 && split[1].length > 0) {
				...
				if (parEle.className.indexOf('navControls') !== -1) {
					addListener = false;
				}
				if (parEle.className.indexOf('topLink') !== -1) {
					addListener = false;
				}
				if (parEle.className.indexOf('messageDetails') !== -1) {
					addListener = false;
				}
				if (parEle.className.indexOf('messageText') !== -1) {
					addListener = false;
				}
				if (parEle.id == 'uix_jumpToFixed') {
					addListener = false;
				}
				if (uix.isSet(parEle.parentElement.className) && parEle.parentElement.className.indexOf('Tabs') !== -1) {
					addListener = false;
				}
				if (addListener) {
	...
	if ('onhashchange' in window) {
		window.onhashchange = uix.init.fixScrollLocation;
	}
};
...
uix.initFunc = function() {
	...
	if (uix.elm.sidebar.length && uix.stickySidebar && uix.windowWidth > uix.sidebarMaxResponsiveWidth && uix.sidebar.visible == 1 && !uix.stickyForceDisable && uix.user.stickyEnableSidebar) {
		$(window).on('load', uix.sidebarSticky.resize);
	}
